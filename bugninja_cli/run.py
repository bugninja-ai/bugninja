...
from bugninja_cli.utils.task_manager import TaskInfo, TaskManager
    ...
    async def run_tasks() -> None:
        ...
        async with TaskExecutor(
            ...
            if all_flag:
                ...
                await _run_single_task(executor, task_manager, task, headless)
            elif multiple:
                await _run_multiple_tasks(executor, task_manager, multiple, headless)
            else:
                _show_no_task_error()
...
async def _run_single_task(
    executor: TaskExecutor, task_manager: TaskManager, task_identifier: str, headless: bool
) -> None:
    """Run a single task.
    ...
    try:
        result = await executor.execute_task(task_info, headless=headless)
...
async def _run_multiple_tasks(
    executor: TaskExecutor, task_manager: TaskManager, task_identifiers: List[str], headless: bool
) -> None:
    ...
    try:
        results = await executor.execute_multiple_tasks(task_infos, headless=headless)
...
async def _run_all_tasks(executor: TaskExecutor, task_manager: TaskManager, headless: bool) -> None:
    """Run all available tasks.
