...
import { TestRun, TestStatus } from '../../test-runs/types';
...
export const useTestRunDetail = (runId?: string): UseTestRunDetailResult => {
  ...
  const transformBackendTestRun = useCallback((backendData: any): TestRun => {
    ...
    const statusMap: { [key: string]: TestStatus } = {
      'PASSED': 'passed' as TestStatus,
      'FAILED': 'failed' as TestStatus,
      'PENDING': 'pending' as TestStatus,
      'RUNNING': 'pending' as TestStatus, // Keep RUNNING as pending to continue polling
      'FINISHED': 'passed' as TestStatus, // Assuming FINISHED means successful completion
      'ERROR': 'failed' as TestStatus
    };
    ...
    // const isReplayRun = backendData.run_type === 'REPLAY'; // Reserved for future use
